...
use anchor_client::{
    ...
    solana_sdk::{commitment_config::CommitmentConfig, pubkey::Pubkey, sysvar},
};
    ...
    loop {
        ...
        while let Ok(r) = rx.recv() {
            ...
            if let Some(c) = load_buf::<zo::Cache>(&buf) {
                tracing::debug!("received cache update");
            ...
            if let Some(c) = load_buf::<zo::SpecialOrders>(&buf) {
                tracing::debug!(
                    "received special orders update: {}",
                    r.value.pubkey
                );
...
fn executer(
    ...
    loop {
        // Ensure that the closures below capture the reference.
        let ms = &ms;
        ...
        // List of (authority, so key, order) that need to be executed.
        let mut pending: Vec<(Pubkey, Pubkey, zo::SpecialOrdersInfo)> =
            Vec::new();
        ...
        for (k, so) in accs.zo_so.read().iter() {
            ...
            for o in so.iter() {
                if o.is_triggered(prices[ms[&o.market].0]) {
                    pending.push((so.authority, *k, *o));
                }
            }
        }

        std::thread::scope(move |s| {
            for (authority, so_key, order) in pending.into_iter() {
                s.spawn(move || {
                    let program = st.program();
                    let (idx, mkt) = ms[&order.market];
                    ...
                    let (margin, control, oo) = {
                        ...
                        match map.get(&authority).copied() {
                            ...
                            None => {
                                ...
                                let margin_key =
                                    margin_pda(&authority, &st.zo_state_pubkey);
                                let margin = program
                                    .account::<zo::Margin>(margin_key)
                                    .unwrap();
                                let control_key = margin.control;
                                let control = program
                                    .account::<zo::Control>(control_key)
                                    .unwrap();
                                let oo_key = control.open_orders_agg[idx].key;

                                accs.zo_trader_accs.write().insert(
                                    authority,
                                    (margin_key, control_key, control),
                                );
                    ...
                    program
                        .request()
                        .args(zo::instruction::ExecuteSpecialOrder {
                            price: order.price,
                            ty: order.ty,
